...
import { ArrowRight, CheckCircle, XCircle, Zap, DollarSign, List, ArrowUpDown, ExternalLink } from 'lucide-react';
...
export default function RouterDemo() {
  ...
  const handleTryRealRouting = async () => {
    setIsRunning(true);
    setRoutingResult(null);
    try {
      const res = await fetch('/api/demo/router-demo', {
        method: 'POST',
      ...
      setSelectedProvider(data.chosenProvider);
      setShowAttempts(true);
    } catch (err: any) {
      alert(err.message || 'Failed to perform routing. Check that backend is running.');
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="glass rounded-xl p-8 max-w-5xl mx-auto">
        ...
        {showAttempts && (
          <div className="mb-8 p-4 bg-white/5 rounded-lg">
            ...
            <button
              onClick={handleTryRealRouting}
              disabled={isRunning}
              className={clsx(
                'w-full mt-4 flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-all',
                isRunning
                  ? 'bg-purple-600 cursor-not-allowed'
                  : 'bg-purple-500 hover:bg-purple-600'
              )}
            >
              <Zap className={clsx('w-4 h-4', isRunning && 'animate-pulse')} />
              {isRunning ? 'Routing...' : 'Try Real Routing'}
            </button>

            {routingResult && (
              <div className="mt-4 p-3 bg-green-500/10 border border-green-500/20 rounded-lg">
                <div className="text-xs text-green-400 font-semibold mb-1">Real Routing Result</div>
                <div className="text-xs text-gray-300">
                  Chosen: <span className="font-mono text-green-300">{routingResult.chosenProvider}</span>
                </div>
                {routingResult.checkoutUrl && (
                  <button
                    onClick={() => window.open(routingResult.checkoutUrl, '_blank')}
                    className="mt-2 w-full flex items-center justify-center gap-2 px-3 py-2 bg-green-500 hover:bg-green-600 text-white rounded text-xs font-medium transition-all"
                  >
